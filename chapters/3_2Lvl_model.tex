...
Migration can be modeled in various ways, depending on factors such as spatial structure, the unit of migration, migration triggers, and destination selection mechanisms.
This chapter extends the base model in a straightforward manner by introducing individuals within groups and allowing them to migrate between neighboring groups, as detailed in Section~\ref{sec:3_model}.
...
\end{equation}
where $r_i^F$ denotes the resource available to agent $i$, $\theta_R$ ($0 < \theta_R < 1$) is the universal threshold shared by all groups, $\theta_F$ is the per-agent threshold obtained by $\theta_F = \tfrac{\theta_R}{N_F / N_R}$, and $p_M$ is a parameter controlling the frequency of migration events.
...
To examine how environmental variability and agent mobility influence the evolution of cooperation, we conduct simulations across a range of parameter settings, as summarized in Table~\ref{tab:3_params}.
...
\addlinespace
$p_M$ & Probability factor multiplied by resource deficit for migration events (direct probability for agents below threshold in Chapter~\ref{ch:4_2D_model}) & $\{0, 0.1, \ldots, 1\}$ \\
\addlinespace
