...
The initial network weight $w_0$ influences the evolution of cooperation at both levels, with smaller $w_0$ promoting higher cooperation rates (Figure~\ref{fig:result3}a,b).
...
The relation weight update rate $\Delta w$ positively affects cooperation, with higher $\Delta w$ promoting cooperation at both levels (Figure~\ref{fig:result3}c,d).
...
This rapid differentiation amplifies the benefits of cooperation and the costs of defection, strengthening selection pressures that favor $C$ strategies.
...
This chapter examined how EV and migration jointly influence the evolution of cooperation using a two-level model with group agents and individual agents.
The results demonstrate that EV promotes cooperation at both levels, though with distinct patterns and mechanisms.

At the group level, cooperation rates increase sharply from approximately $0.5$ to $0.7$ when EV is introduced ($p_{EV} > 0$), and this pattern is independent of migration intensity ($p_M$).
At the individual level, cooperation rates are suppressed below $0.1$ without EV ($p_{EV} = 0$) but increase substantially when EV is present.
Unlike group-level cooperation, individual-level cooperation is strongly affected by migration: moderate migration ($p_M \approx 0.1$) maximizes cooperation, while excessive migration reduces it.

Ablation experiments reveal an asymmetric influence between levels: group-level processes contribute to individual-level cooperation, whereas individual-level processes have minimal effect on group-level outcomes.
EV promotes group-level cooperation by creating temporal equity in resource distribution, which enables all groups to experience both resource-rich and resource-poor periods, thereby increasing the long-term value of cooperative relationships.
At the individual level, EV maintains resource disparities between groups, allowing cooperators to form stable relationships and outcompete defectors who remain isolated from profitable interactions.

These findings extend the base model in Chapter~\ref{ch:2_base_model} by demonstrating that EV can promote cooperation even when migration is incorporated.
Furthermore, the results reveal that moderate migration enhances cooperation by facilitating the formation of cooperative networks, while excessive migration disrupts these structures.
The next chapter, Chapter~\ref{ch:4_2D_model}, examines whether similar patterns emerge in a two-dimensional spatial structure without explicit group boundaries.
