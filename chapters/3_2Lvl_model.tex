...
First, regarding spatial structure, previous studies have primarily employed either patch structures \cite{Dugatkin1991}, where agents move between discrete locations, or 2-dimensional continuous spaces \cite{Vainstein2001, Vainstein2007}, where agents move across a grid or plane.
...
The 2-dimensional spatial approach is examined in Chapter~\ref{ch:4_2D_model}.
...
Regarding migration triggers and destination selection mechanisms, individuals migrate, triggered by resource scarcity, toward resource-rich areas, as this research conceptualizes EV as fluctuations in resource availability.

These design choices result in a 2-level hierarchical structure with group agents and individual agents.
...
While Traulsen and Nowak (2006) \cite{Traulsen2006} is widely recognized as a seminal study on the evolution of cooperation in a 2-level structure, the model presented in this chapter adopts a fundamentally different approach.
...
In contrast, the 2-level model in this chapter is structured such that the payoff rules of the game (see Subsection \nameref{sec:game}) inherently define $D$ as the advantageous strategy at both the individual and group levels.
...
We consider the model composed of multiple regional groups, each inhabited by a number of individuals who may migrate between the groups in response to environmental or social pressures.
Within this world, the population is represented by group agents and individual agents, and their interaction (see Subsection \nameref{sec:game}) and migration (see Subsection \nameref{sec:migration}) are governed by the geographical and interaction structures (see Figure~\ref{fig:3_model}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/3/Model}
  \caption[Illustration of the model structure]{
Illustration of the model structure.
Regional groups are arranged on a circular geographic structure, where the \textit{Source of Resources} (SoR) stochastically moves between groups.
...
An interaction structure is represented by a weighted, undirected and complete graph whose edge weights are dynamic while its topology remains fixed.
Importantly, the geographical structure and the interaction structures are completely independent; the geographical distance between nodes does not influence the probability of their interaction or the weights of their relationships.
...
In a social context, this weight represents the level of mutual trust accumulated through repeated interactions.

Given these structural foundations, the simulation proceeds in four stages: EV, game, migration, and strategy update.
Game and strategy update occur at both the group and individual levels, in that order.
...
To represent this type of variability, we define the \textit{Source of Resources} (SoR) as a dynamic point corresponding to the most resource-abundant region.
...
This setup reflects collective activities in MSA societies at two distinct scales: individual-level games correspond to localized cooperation such as large-game hunting or commons maintenance, whereas group-level games represent broader inter-communal interactions like long-distance trade.
...
Because agents benefit from being connected to $C$ but not to $D$, the direction of change depends on the combination of strategies  \cite{Liu2020, Xiong2024}.
...
This process models success-biased cultural transmission, where agents improve survival by imitating the behavioral patterns of more prosperous agents (e.g., successful hunters, thriving neighboring regions) and resetting their social ties.
...
As the principal performance indicators, we calculate the cooperation rates $\phi_C^R$ and $\phi_C^F$, defined as the average proportions of groups and individuals, respectively, employing strategy $C$.
Specifically, $\phi_C^R$ is the mean of the proportions of the $N_R$ groups employing strategy $C$ at the end of each generation (following the strategy update stage), averaged over the last $5000$ generations and $100$ independent runs.
$\phi_C^F$ is calculated in the same manner, using the proportions of the $N_F$ individuals employing strategy $C$.
...
\addlinespace
$p_{EV}$ & Probability of SoR shift; controlling the intensity of EV & $\{0, 0.1, \ldots, 1\}$ \\
\addlinespace
...
\addlinespace
$p_M$ & Probability factor multiplied by resource deficit for migration events, controlling the individual mobility & $\{0, 0.1, \ldots, 1\}$ \\
\addlinespace
...
\section{Results}\label{sec:3_results}
...
These stable cooperators repeatedly interact with each other, gradually strengthening their mutual relationships, while defectors' relationships decline, isolating them from profitable interactions.
Of course, groups may subsequently become resource-poor.
...
While the group agents are the same as in Chapter \ref{ch:2_base_model}, the individual agents newly introduced in this chapter are allowed to migrate between neighboring groups in response to resource scarcity.
...
In addition, lower initial relationship strength ($w_0$) and faster updating of relationship weights ($\Delta w$) further facilitate the formation and maintenance of cooperative networks.
...
The next chapter, Chapter~\ref{ch:4_2D_model}, examines the effects of EV and migration in a 2-dimensional spatial structure without explicit group definitions.
