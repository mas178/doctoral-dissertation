...
While numerous factors that promote cooperation have been proposed, this dissertation focuses on environmental variability (EV) as a potentially crucial yet understudied driver.
This chapter reviews the literature on the evolution of cooperation, explains why we focus on EV, outlines the research objectives and approach, and provides an overview of the dissertation's structure.

\section{Theoretical background}\label{sec:background}

Darwin's theory of natural selection in \textit{On the Origin of Species} (1859) \cite{Darwin1859} includes the principle that nature favors traits that increase individual fitness---the ability to survive and reproduce.
...
In recent years, research has increasingly shifted toward examining cooperation under more specific circumstances and mechanisms \cite{Szabo2007, Perc2010, Zaggl2014, Perc2017, West2021, Jusup2022}.
These studies investigate how factors such as reputation, social norms, memory, complex network structures, learning mechanisms, and EV shape the evolution of cooperation.
...
\textit{Modern human behavior} refers to a suite of traits characteristic of \textit{Homo sapiens}, including abstract thinking, symbolic expression, complex planning, language, art, and crucially, large-scale cooperation and ultrasociality.
...
While there is broad consensus on when and where these behaviors originated, the mechanisms driving their emergence remain enigmatic despite various proposed hypotheses.

Among various hypotheses, the variability selection hypothesis (VSH), proposed by Potts (1996, 1998) \cite{Potts1996, Potts1998}, suggests that EV was a primary driving force in human evolution.
According to this hypothesis, intensified environmental fluctuations during the MSA in Africa favored ``versatilists'', those capable of rapid adaptation to new environments, over ``specialists'', who adapt to specific environments, or ``generalists'', who adapt across a range of environments.
In this context, EV encompasses changes in landscape dynamics (such as land-lake oscillations), climate fluctuations (such as arid-moist climate oscillations), and variations in flora and fauna, ultimately leading to the unpredictability of resource availability.
...
These theories emphasize the role of social contexts, as suggested by the social brain hypothesis (SBH) \cite{Whiten1988, Dunbar1998, Barrett2007, Grove2008, Knight2011, Hayes2014, Faith2021, Dunbar2024}, and consider other factors such as dietary influences \cite{DeCasien2017, Grabowski2023}.
...
In particular, the fact that large-scale complex cooperation is a component of modern human behavior raises the question of whether EV also played a role in the evolution of cooperation.
This speculative link provides the first motivation of this dissertation.
...
Extrinsic EV can be represented through several models, including fluctuations in the population that the environment can sustain \cite{Brockhurst2007, Miller2015}, stochastic variations in payoff matrices or game rules \cite{Gokhale2016, Stojkoski2021, Kroumi2021}, and variability in learning and information transmission mechanisms \cite{Borg2012,Assaf2013}.
...
Intrinsic EV, in contrast, refers to EV in which interactions in a system modify the environment, and environmental feedback in turn shapes the system's behavior \cite{Szolnoki2017, Huang2025}.
...
The constructive approach is a methodology that seeks to understand a phenomenon by artificially constructing systems that reproduce the phenomenon and analyzing the behavior.
...
We then identified the relationship between cooperation and EV as the central focus of this dissertation and posed two research questions: whether EV promotes cooperation, and if so, how it does.
...
Chapter \ref{ch:3_2Lvl_model} (\nameref{ch:3_2Lvl_model}) extends the base model by introducing a 2-level structure with individual-level migration.
...
Chapter \ref{ch:4_2D_model} (\nameref{ch:4_2D_model}) addresses this limitation by employing a 2-dimensional spatial structure that is widely used in the literature on cooperation with migration.
In this model, without assuming explicit group structures, as defined in Chapters \ref{ch:2_base_model} and \ref{ch:3_2Lvl_model}, we investigate how EV influences cooperation among individuals moving across a 2-dimensional space.
...
\midrule
Agent &
An autonomous unit in simulation models that possesses a behavioral strategy, perceives its environment and takes actions.
In Chapter \ref{ch:2_base_model}, agents represent groups.
In Chapter \ref{ch:3_2Lvl_model}, two levels of agents exist: groups and individuals within groups.
In Chapter \ref{ch:4_2D_model}, agents represent individuals. \\
\addlinespace
Individual &
An agent representing the unit of migration.
Due to this definition, individuals are not necessarily single persons but may represent small units such as families.
Individuals are modeled as agents in Chapters \ref{ch:3_2Lvl_model} and \ref{ch:4_2D_model}, but not in Chapter \ref{ch:2_base_model}.\\
\addlinespace
Group &
A collection of individuals sharing a common geographical location.
Groups are modeled as agents in Chapters \ref{ch:2_base_model} and \ref{ch:3_2Lvl_model}, but not in Chapter \ref{ch:4_2D_model}.\\
\addlinespace
Cooperation &
Altruistic behavior that appears to benefit other agents at a cost to an acting agent.
Cooperation ($C$) and its alternative behavior, defection ($D$), are used as behavioral strategies of an agent in game-theoretic models.\\
\addlinespace
Environmental Variability (EV) &
Temporal fluctuations in the spatial distribution of resource availability.
This dissertation focuses on extrinsic EV, which occurs independently of human activity, reflecting unpredictable climate and landscape fluctuations during the MSA in Africa.
In the later chapters, EV is usually modeled through the stochastic movement of \textit{Source of Resources} (SoR), focal points of resource abundance, and the resulting shifts in resource distribution.\\
\addlinespace
Migration and Mobility &
Migration refers to spatial movement of individuals in response to resource availability or environmental conditions, while mobility refers to the ability to migrate.
In Chapter \ref{ch:2_base_model}, migration is not incorporated.
In Chapter \ref{ch:3_2Lvl_model}, individuals migrate between groups.
In Chapter \ref{ch:4_2D_model}, individuals migrate across a 2D space.\\
\bottomrule
\end{tabular}
\end{table}
