...
First, groups within the complex social environment described by the social brain hypothesis (SBH) are nested in a series of fractal-structured networks \cite{Bird2019, Dunbar2020, Dunbar2024a}.
...
In this model, each agent represents a single group and has a strategy of either cooperation or defection (in this chapter, ``C'' represents a cooperation strategy or a cooperator, while ``D'' represents a defection strategy or a defector).
...
\textcolor{red}{ToDo: prime node -> SoR.}
    }
    \label{fig:model}
\end{figure}
...
To assess the effect of EV on the evolution of cooperation we compared the average cooperation rates across different parameters controlling RV and UV, along with other factors (Table \ref{tab:parameters}).
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|l|c|}
        ...
        $\sigma_\theta$ & SD of the noise term of UV & $\{0.0, 0.1, 02\}$ \\
        \hline
...
\textcolor{red}{ToDo: Review this subsection.}
...
These differences arise from two key factors: the extent to which EV generates fluctuations in strategy distribution through mutation and reformation, and the coevolution of cooperation and network structure.
RV effectively promotes both factors, whereas UV generates insufficient fluctuations and disrupts network heterogeneity.

A limitation of this model is that agents cannot migrate between locations.
In reality, migration toward resource-rich areas is a natural response to environmental change, and such movement would influence both the evolution of cooperation and network structure.
The following chapters address this limitation by introducing mobility: Chapter \ref{ch:3_2Lvl_model} extends this model with individual-level migration within a two-level group structure, and Chapter \ref{ch:4_2D_model} examines cooperation among mobile individuals in a two-dimensional spatial structure.
