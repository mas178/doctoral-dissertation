...
Dugatkin and Wilson (1991) \cite{Dugatkin1991} conducted pioneering work in this area, demonstrating that the effectiveness of Axelrod's Tit-for-Tat strategy \cite{Axelrod1981} can be undermined by mobile defectors (Rover strategy) when migration costs are low, although their model employed a patch structure rather than a 2D space.
...
They demonstrated that mobility can have contrasting effects: under certain conditions, it promotes cooperation by facilitating cooperator clustering and separation from defectors, while under other conditions, it hinders cooperation by inducing strategic instability.
...
This chapter addresses this gap within Vainstein's framework by modeling EV as randomly moving resource-rich spots (\textit{Sources of Resources} (SoRs)) across a 2D space and agent mobility as resource-seeking migration.
...
In this chapter, we developed a multi-agent simulation model to examine how agent mobility and EV jointly influence the evolution of cooperation in spatially structured populations.
Due to the limited availability of archaeological data on spatial resource distributions and hominin behavioral patterns during the Middle Stone Age (MSA), we adopted an abstracted approach that prioritizes the identification of fundamental mechanisms over reproducing specific historical scenarios.
This model incorporates four key processes:
...
We define an SoR as a focal point that generates spatial resource gradients in the environment, and these SoRs serve as simplified representations of the natural foraging areas commonly found near rivers, lakes, and coastlines.
...
\addlinespace
$p_M$ & Probability of migration for agents below threshold & $\{0, 0.1, \ldots, 1\}$ \\
\addlinespace
...
While the 2-level model in Chapter \ref{ch:3_2Lvl_model} is a natural extension of the base model, its unique structure limits direct comparison with previous studies on cooperation and migration.
...
The next chapter, Chapter~\ref{ch:5_conclusion}, synthesizes and compares the findings from all three models to identify common mechanisms and discuss the broader implications and limitations for understanding the evolution of cooperation under EV.
