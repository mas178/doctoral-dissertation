...
The base model (スライド＃８) でSoR③が仮に左に移動すると② (0.8) はどこに移動するのか、図を見てよくわからなかったので、最終発表でもっと説明があるとよい。
...
平均協力率の計算をする際に１万世代の後半の５千世代のものを使って計算しているが、その根拠を議論するとよい。また、別の区間を用いた場合、結論が変わるかを少し議論するとよい。
...
ご指摘ありがとうございます。
以下のパラグラフを追加しました。(Ch.3 3.1 Model p.20)
...
In contrast, the 2-level model in this chapter is structured such that the payoff rules of the game (see Subsection \textbackslash nameref\{sec:game\}) inherently define $D$s as the advantageous strategy at both the individual and group levels.
...
予備審査の説明では誤解を与えてしまったかも知れませんが、Chapter 4 は Chapter 3 の拡張ではありません。
...
しかし、Chapter 2 の目的は、地理的隔離が存在する条件下で環境変動が協力に与えるメカニズムをできるだけ基礎的な条件の元で解明し、研究のスタートラインを提供することにあります。
円環構造は、地理的隔離を表現する最も単純な枠組みであり、境界効果を排除しすべての地域の条件を均一に保つため、妥当な枠組みであると考えています。

Ch.2 では、「our model adopts a highly abstracted approach, aiming to reveal the general effects of EV based on reasonable assumptions while excluding specific details. (Ch.2 p.8)」や「the one-dimensional spatial structure is used for simplicity (Ch.2 p.9)」と説明しています。
...
また、モデルの実装内容については、Appendix にて整理しました (p.60 Appendix -- Table A.1: Detailed comparison of the three simulation models)。
...
仰る通り理論的には第2章と第3章のGroup-Levelは同等のModelであり、ご指摘のような比較ができたほうが良かったのですが、第2章の研究の経験を踏まえて、第3章の実装方法を第2章から少し変えてしまったため、単純な比較は難しくなっています。
代わりに、第3章内で、Ablation experiment (p.25) を行い、Figure 3.2ac  (ノーマルな実行結果) と Figure 3.4ac (Individual-level のプロセスを非活性化し、Group-level のプロセスだけを実行した結果) とを比較し、Group-Level の実行結果が変わらないことを確認することで、第2章と第3章のGroup-Levelの同等性が間接的に示しました。
